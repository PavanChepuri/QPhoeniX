...
// 1) Returns all unique option expiry dates for an underlying (CE/PE only).
QVector<QDate> DataManager::optionExpiriesFor(const QString& underlying)  {
    QVector<QDate> out;
    QSet<QDate> seen;
    for (const auto& d : m_allInstruments) { // your in-memory list from instruments.csv
        if (d.segment == "NFO-OPT" && d.name.compare(underlying, Qt::CaseInsensitive) == 0) {
            if (!seen.contains(d.expiry)) { seen.insert(d.expiry); out.push_back(d.expiry); }
        }
    }
...
// 2) Next weekly = earliest expiry >= today.
QDate DataManager::nextWeeklyFrom(const QVector<QDate>& expiries, const QDate& today)  {
    for (const auto& e : expiries) if (e >= today) return e;
    return QDate(); // none
}

// 3) Monthly = last expiry within today's month (if none, fall back to latest in list).
QDate DataManager::monthlyForThisMonth(const QVector<QDate>& expiries, const QDate& today)  {
    QDate pick;
    for (const auto& e : expiries) if (e.year()==today.year() && e.month()==today.month()) pick = e;
    if (pick.isValid()) return pick;
    return expiries.isEmpty() ? QDate() : expiries.last();
}
...
void DataManager::loadInstrumentsFromFile(const QString &filename)
{
    ...
    // 1) Keep indices, drop prior NFO so we can rebuild a clean set.
    {
        int removed = 0;
    ...
    QTextStream in(&file);
    QString header = in.readLine(); // consume header
    int linesRead = 0;
    int parsed = 0;

    // 2) Parse all NIFTY/BANKNIFTY NFO instruments first (don’t filter by weekday).
    QVector<InstrumentData> candidates;
    candidates.reserve(200000);

    auto isNiftyOrBank = [](const InstrumentData &d) -> QString {
        const QString n = d.name.trimmed().toUpper();
        if (n == "NIFTY")      return "NIFTY";
        if (n == "BANKNIFTY")  return "BANKNIFTY";
        return QString(); // reject NIFTYNXT50, FINNIFTY, MIDCPNIFTY, etc.
    };
    ...
    while (!in.atEnd()) {
        ...
        if (d.segment.isEmpty()) continue;     // bad/empty row
        if (!d.expiry.isEmpty() && !d.expiryDate.isValid()) {
            // be strict—skip malformed expiries
            continue;
        }

        // Only collect NFO Options/Futures for NIFTY/BANKNIFTY
        if (d.segment == "NFO-OPT" || d.segment == "NFO-FUT") {
    ...
    if (candidates.isEmpty()) {
        qWarning() << "No NIFTY/BANKNIFTY NFO rows found in file. Aborting load.";
        emit allInstrumentsDataUpdated();
        return;
    }

    // 3) Temporarily insert all candidates into the map so our helpers can scan expiries.
    int inserted = 0;
    for (const auto &d : candidates) {
        m_instruments.insert(d.instrumentToken, d);
        ++inserted;
    }
    qDebug() << "Inserted candidates into map:" << inserted;

    // 4) Compute dynamic expiries from **what exists** in the file (future-proof).
    ...
    qInfo().noquote() <<
        QString("Dynamic expiry selection -> "
                "NIFTY [weekly=%1, monthly=%2], BANKNIFTY [weekly=%3, monthly=%4]")
    ...
    // 5) Prune: keep only options for (weekly OR monthly) expiries, and keep
    //           FUTURES only for the monthly expiry (current month) per underlying.
    auto shouldKeep = [&](const InstrumentData &d) -> bool {
        const QString u = isNiftyOrBank(d);
        if (u.isEmpty()) return false; // keep only NIFTY/BANKNIFTY derivatives

        if (d.segment == "NFO-OPT") {
            if (!d.expiryDate.isValid()) return false;
            if (u == "NIFTY")     return (d.expiryDate == niftyWeekly) || (d.expiryDate == niftyMonthly);
            if (u == "BANKNIFTY") return (d.expiryDate == bankWeekly)  || (d.expiryDate == bankMonthly);
            return false;
        }
        if (d.segment == "NFO-FUT") {
            if (!d.expiryDate.isValid()) return false;
            if (u == "NIFTY")     return d.expiryDate == niftyMonthly;
            if (u == "BANKNIFTY") return d.expiryDate == bankMonthly;
            return false;
        }
        return false;
    };

    int before = m_instruments.size();
    for (auto it = m_instruments.begin(); it != m_instruments.end(); ) {
        const InstrumentData &d = it.value();
        if (d.segment.startsWith("NFO")) {
            if (!shouldKeep(d)) { it = m_instruments.erase(it); continue; }
        }
        ++it;
    }
    int after = m_instruments.size();
    qInfo().noquote() << QString("Pruned instruments: before=%1, after=%2, removed=%3")
                             .arg(before).arg(after).arg(before - after);

    // 6) Persist a slim CSV snapshot for debugging/inspection (your existing util).
...
// Returns the earliest expiry >= fromDate for the given underlying (weekly by convention).
QDate DataManager::nearestWeeklyExpiry(const QString& underlying, const QDate& fromDate) const
{
    const auto map = getAllInstruments(); // QHash<QString, InstrumentData>
    QDate best;                            // invalid by default

    for (const auto& inst : map) {
        if (inst.segment != "NFO-OPT") continue;
        if (inst.name != underlying)      continue;
        if (!inst.expiryDate.isValid())   continue;
        if (inst.expiryDate < fromDate)   continue;
        if (!best.isValid() || inst.expiryDate < best) best = inst.expiryDate;
    }
    return best; // may be invalid if nothing found
}

// Returns the "monthly" expiry in the same month as fromDate (or the next month that has any options).
QDate DataManager::monthlyExpiryInSameMonth(const QString& underlying, const QDate& fromDate) const
{
    const auto map = getAllInstruments();

    // Gather all expiries >= fromDate for this underlying.
    QSet<QDate> expiries;
    for (const auto& inst : map) {
        if (inst.segment != "NFO-OPT") continue;
        if (inst.name != underlying)    continue;
    ...
    if (expiries.isEmpty()) return QDate(); // nothing found

    // Find the earliest month (>= fromDate's month) that has at least one expiry.
    // Then pick the max (last) expiry within that month -> monthly expiry.
    const int fromYm = fromDate.year() * 100 + fromDate.month();

    // step 1: find target year-month
    int targetYm = -1;
    ...
    if (targetYm == -1) {
        // no expiries in or after this month (unlikely if we had any), return latest overall
        QDate latest;
        for (const auto& d : expiries) if (!latest.isValid() || d > latest) latest = d;
        return latest;
    }

    // step 2: choose the last expiry in that target month
    QDate monthly;
    for (const auto& d : expiries) {
        const int ym = d.year() * 100 + d.month();
...
// Returns all NFO options for the given underlying & expiry.
                                                                   ...
                                                                   const QDate& expiry) const
{
    ...
    for (const auto& inst : map) {
        ...
        if (inst.name != underlying)    continue;
        if (!inst.expiryDate.isValid()) continue;
        if (inst.expiryDate != expiry)  continue;
        out.push_back(inst);
    }
    return out;
}
