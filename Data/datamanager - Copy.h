...
private:
    ...
    QVector<QDate> optionExpiriesFor(const QString& underlying);
    // 2) Next weekly = earliest expiry >= today.
    QDate nextWeeklyFrom(const QVector<QDate>& expiries, const QDate& today);
    // 3) Monthly = last expiry within today's month (if none, fall back to latest in list).
    QDate monthlyForThisMonth(const QVector<QDate>& expiries, const QDate& today);
